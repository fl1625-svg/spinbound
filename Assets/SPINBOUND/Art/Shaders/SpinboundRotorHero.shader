Shader "SPINBOUND/Rotor Hero"
{
    Properties
    {
        ...
        _MetalColor("Brushed Metal Color", Color) = (0.24,0.30,0.38,1)
        _MechanismColor("Mechanism Color", Color) = (0.035,0.05,0.075,1)
        _EnergyColor("Energy Glass Color", Color) = (0.12,0.62,1.0,1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                float roleSmoothness = saturate(_Smoothness + metalMask * 0.28 + energyMask * 0.18);
                float roleMetallic = saturate(_Metallic + metalMask * 0.85 + mechanismMask * 0.42);
                ...
                float speed = saturate(_SpeedState * 0.5);
                float energyPulse = energyMask * _EmissionStrength * (0.62 + speed * 0.58);
                float ceramicEdge = ceramicMask * speed * 0.06;

                float3 color = baseColor * (ambient * 0.72 + direct);
                color += specular * mainLight.color * lerp(0.08, 0.88, roleMetallic);
                color += _RimColor.rgb * rim * (0.72 + speed * 0.28);
                color += _EnergyColor.rgb * (energyPulse + ceramicEdge);
